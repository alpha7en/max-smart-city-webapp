...
логи — `docker compose logs -f meter-reader`. Порт 8000 открыт только на 127.0.0.1 хоста.
...
- `serial_number` сверяется с номером сохранённого счётчика (и сохраняется у нового).
...
- `confidence` сервиса не используется (он почти всегда 0.95), кроме случая, когда он ниже 0.5. Бот сам просит
  «Проверьте цифры внимательно», если `meter_type` не совпал с выбранным счётчиком, если цифр больше, чем на табло
  такого типа (вода и газ 5 + 3, свет 6 + 2), или если это не водомер.
