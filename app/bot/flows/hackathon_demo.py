...
# Реальные дома на длинных центральных улицах (номера точно есть); квартира выбирается случайно.
# Город → дома. Сначала выбираем 2 разных города, чтобы Москва не выпадала через раз.
HOUSES = {
    "г. Москва": ("ул. Тверская, д. 8", "ул. Арбат, д. 35", "ул. Профсоюзная, д. 43", "Ленинский пр-кт, д. 45",
                  "Кутузовский пр-кт, д. 26", "ул. Большая Полянка, д. 28", "ул. Вавилова, д. 48",
                  "пр-кт Мира, д. 102", "Ленинградский пр-кт, д. 75", "Мичуринский пр-кт, д. 6"),
    "г. Санкт-Петербург": ("Невский пр-кт, д. 100", "Лиговский пр-кт, д. 50", "Московский пр-кт, д. 150",
                           "ул. Марата, д. 40", "Каменноостровский пр-кт, д. 40", "ул. Савушкина, д. 10"),
    "г. Казань": ("ул. Баумана, д. 58", "ул. Декабристов, д. 85", "пр-кт Победы, д. 100", "ул. Пушкина, д. 20"),
    "г. Екатеринбург": ("ул. Малышева, д. 51", "пр-кт Ленина, д. 50", "ул. Вайнера, д. 15"),
    "г. Новосибирск": ("Красный пр-кт, д. 25", "ул. Ленина, д. 12", "ул. Кирова, д. 44"),
    "г. Нижний Новгород": ("ул. Большая Покровская, д. 30", "ул. Родионова, д. 40"),
    "г. Самара": ("ул. Куйбышева, д. 100", "ул. Ленинградская, д. 50"),
    "г. Ростов-на-Дону": ("ул. Большая Садовая, д. 50", "Ворошиловский пр-кт, д. 40"),
    "г. Краснодар": ("ул. Красная, д. 120", "ул. Северная, д. 300"),
    "г. Воронеж": ("пр-кт Революции, д. 30", "ул. Плехановская, д. 20"),
    "г. Пермь": ("ул. Ленина, д. 60", "Комсомольский пр-кт, д. 34"),
    "г. Уфа": ("пр-кт Октября, д. 100",),
    "г. Челябинск": ("пр-кт Ленина, д. 50", "ул. Кирова, д. 110"),
    "г. Омск": ("ул. Ленина, д. 20",),
    "г. Ярославль": ("ул. Свободы, д. 50",),
    "г. Тула": ("пр-кт Ленина, д. 80",),
    "г. Калининград": ("Ленинский пр-кт, д. 30",),
    "г. Владивосток": ("ул. Светланская, д. 50",),
    "г. Сочи": ("Курортный пр-кт, д. 50",),
    "г. Тверь": ("ул. Советская, д. 30",),
    "г. Саратов": ("ул. Московская, д. 100",),
    "г. Иркутск": ("ул. Карла Маркса, д. 20",),
}
...
_ADDRESSES = AddressService()  # локальный разбор без сети: дома из списка, DaData не нужна
...
def random_addresses(rnd: random.Random, n: int = ADDRESSES) -> list[str]:
    """n адресов в разных городах в формате «г. Москва, ул. Арбат, д. 35, кв. 32»: реальный дом, случайная квартира."""
    return [f"{city}, {rnd.choice(HOUSES[city])}, кв. {rnd.randint(1, 200)}"
            for city in rnd.sample(sorted(HOUSES), n)]
