...
| Демо-профиль (только для хакатона) | команда `/demo_profile` (есть в списке команд бота) у пользователя без профиля создаёт вымышленный профиль: случайные ФИО и телефон, 2 адреса, 4–6 счётчиков и показания за полгода. Если профиль уже есть, бот просит сначала удалить его. Код отдельно: `app/bot/flows/hackathon_demo.py` | убрать модуль после хакатона (список файлов — в его docstring) |
...
- Тестовый профиль (при `HACKATHON_DEMO_PROFILE=true`): вместо регистрации отправьте `/demo_profile` (команда
  есть в меню команд бота). Бот создаст вымышленный профиль со случайными ФИО и телефоном, двумя случайными
  адресами, 4–6 счётчиками с историей показаний за полгода; у одного счётчика срок поверки меньше 30 дней.
  Если профиль уже есть: «Профиль» → «Удалить мои данные», затем снова `/demo_profile`.
