...
Один сервис `app` (Python 3.12, FastAPI), один контейнер. В одном процессе работают:
...
flowchart LR
  ...
  F -.-> REC["распознавание: RECOGNIZER_URL или демо"]
...
| `RECOGNIZER_URL` | URL сервиса распознавания показаний | пусто: демо-распознавание | нет |
...
| Распознавание показаний | клиент готов, сервис внешний | `POST RECOGNIZER_URL` (multipart: `file`, `meter_type`, `tariffs`) → `{"values":{"t1":…},"serial":…,"confidence":…}`. Сервис команды живёт в отдельном контейнере и в этот репозиторий не входит. Без URL работает демо-распознавание |
...
|---|---|---|
| Распознавание без `RECOGNIZER_URL` | цифры подставляются: прошлое показание плюс правдоподобный прирост. Бот пишет «Демо-распознавание: значение подставлено» | задать `RECOGNIZER_URL` сервиса распознавания (клиент `HttpRecognizer` готов) |
...
- Ошибка распознавания: пришлите фото с подписью «ошибка». Работает только в демо-распознавании.
...
- Адрес решает, кем вы станете. Первый зарегистрированный по адресу становится собственником. Если по адресу
  из примера уже кто-то зарегистрирован, вы попадёте в сценарий «нет прав». Чтобы подавать показания,
  укажите другую квартиру.
...
| 7 | Выбрать тип и адрес | «Смотрим на фото…», значение с пометкой демо-распознавания, кнопки «Отправить», «Исправить», «Переснять», «Отмена» |
...
«Сервис временно недоступен», значит недоступен его бэкенд.
...
- `/start` или «меню» посреди подачи → «Предыдущую подачу отменили» и меню. Посреди регистрации бот
...
- Реальное распознавание, сроки, счета, права и передача в УК пока смоделированы (см. таблицу выше).
...
(`tests/fakes.py`). Сеть и токен им не нужны.
