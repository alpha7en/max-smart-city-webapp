...
| ФГИС «Аршин» (Росстандарт, реестр поверок) | реальная при `ARSHIN_MODE=live` | после первой подачи с заводским номером бот ищет поверку в официальном API `GET /eapi/vri` (руководство «Внешние публичные интерфейсы» v2.2): фильтр по типу СИ, марке и модели с фото, последняя запись в группе. Нашли уверенно — «Поверка по данным ФГИС «Аршин»: до …», ссылка на карточку, вопрос о дате не задаётся; несколько подходящих приборов — «Это ваш счётчик?» с кнопками; не нашли — честный текст и вопрос о дате из паспорта. Ждём не дольше 6 с, остальное доделывается в фоне; раз в сутки планировщик обновляет до 10 счётчиков. `fixtures` — демо-данные ФГИС для показа без сети, в сообщениях «демо-данные, не из реестра» |
...
- ФГИС «Аршин»: API доступен только с российских IP (из облачной среды разработки не проверялся, тесты — на фикстурах
  по документации); лимит 2 запроса в секунду (бот держит паузу 0,6 с, до 4 запросов на проверку, кэш: найдено —
