...
- Мини-приложение: `https://maxsmartcity.ru/`. Открывается внутри MAX
...
5. Распознавание → проверка: «Отправить», «Исправить», «Переснять», «Отмена». Показание выводится как прочитано
   («2168», без дописанных нулей), сомнения сервиса (блики, размыто, мало цифр) — конкретными предупреждениями.
   Не разобрали — бот называет причину и предлагает переснять или ввести вручную.
6. Серийный номер обязателен: сверяется с сохранённым; если его нет ни у счётчика, ни на фото, бот просит
   переснять или ввести номер (формат проверяется по типу, `app/domain/serials.py`).
7. Бот проверяет формат, сравнивает с прошлым показанием и приростом, спрашивает о замене, если за месяц
...
допишите в `.env` ключи Yandex Cloud (сервисный аккаунт с ролью `ai.languageModels.user`):
...
| `HACKATHON_DEMO_PROFILE` | только для хакатона: после регистрации предложить демо-профиль, команда `/demo_profile` | `true` | нет |
...
|---|---|---|
| Распознавание без ключей Yandex Cloud | цифры подставляются: прошлое показание плюс правдоподобный прирост. Бот пишет «Демо-распознавание: значение подставлено» | вписать ключи YC (см. [Реальное распознавание](#реальное-распознавание-показаний)) |
...
| Дата поверки | из ФГИС «Аршин» по заводскому номеру (`ARSHIN_MODE=live`), иначе со слов пользователя. Если её нет, `/demo` ставит модельную дату (+20 дней) с пометкой «смоделирована для демо» | живая проверка ФГИС с российского IP (см. ограничения) |
| Демо-профиль (только для хакатона) | после регистрации бот спрашивает «Сгенерировать демонстрационный профиль?» и добавляет 2 вымышленных адреса, 5 счётчиков и показания за полгода с пометкой «вымышленный». Код отдельно: `app/bot/flows/hackathon_demo.py` | убрать модуль после хакатона (список файлов — в его docstring) |
...
SQLite, файл `data/bot.db` (режим WAL). Схема версии 1 лежит в `app/schema.sql`, дальше миграции `app/db.py`
(`MIGRATIONS`, `PRAGMA user_version`): 2 — приглашения, 3 — поверка по ФГИС. Применяются при старте, данные сохраняются.
...
- `meters`: тип, тарифность, серийный номер, дата поверки и её источник (`user`/`model`/`arshin`, запись ФГИС и время проверки).
  `arshin_cache`: ответы ФГИС «Аршин» по номеру и типу. `readings`: показания за месяц в тысячных долях,
...
- `invites`: одноразовые приглашения жильцов. `photos`: временные фото, `bills`: демо-счета,
  `notifications`: какие уведомления уже отправлены, `kv`: маркер long polling и служебные отметки.
...
- Демо-профиль (при `HACKATHON_DEMO_PROFILE=true`): сразу после регистрации бот спросит «Сгенерировать
  демонстрационный профиль?». «Да» добавит два вымышленных адреса (Москва, Тверская 12, кв 45 и Истра, Садовая 3),
  пять счётчиков, среди них двухтарифный свет и горячая вода со сроком поверки через 25 дней, и историю показаний
  за полгода. Отказались — `/demo_profile` предложит снова. Повторно профиль не создаётся.
...
- `/demo` (при `DEMO_MODE=true`) присылает настоящие уведомления с рабочими кнопками: «О подаче», «О поверке»,
  «О счёте». «О поверке» требует хотя бы один счётчик. «Поверка в ФГИС» (если `ARSHIN_MODE` не `off`) проверяет
  счётчик с заводским номером.
- Ошибка распознавания: пришлите фото с подписью «ошибка». Работает только в демо-распознавании; с настоящим распознаванием
...
- «Нет прав»: первый зарегистрированный по адресу — собственник. Второй аккаунт MAX на тот же адрес получит
  «Запросить доступ», собственнику придёт запрос «Разрешить» / «Отклонить». При `DEMO_MODE=true` есть ещё
  «Открыть доступ (демо)»: одобряем за собственника, и можно сразу подавать показания.
...
| 4 | Написать адрес, например «г. Москва, ул. Арбат, д. 47, корп. 1, кв. 32» (или свою квартиру) | «Мы поняли так: …» и кнопки «Да» / «Нет, ввести иначе» |
...
| 7 | Выбрать тип и адрес; если номер на фото не виден — «Ввести номер», например `18-452178` | «Смотрим на фото…», значение с фото (без ключей YC — с пометкой демо-распознавания), кнопки «Отправить», «Исправить», «Переснять», «Отмена» |
| 8 | «Отправить» | «Готово! Записали …», «Передача в управляющую компанию в MVP смоделирована», срок поверки из ФГИС «Аршин» или вопрос о дате |
...
- `/start` или «меню» посреди подачи → «Предыдущую подачу отменили» и меню; посреди регистрации — продолжение с того же места.
- Новое фото посреди подачи → «Взяли новое фото». Показание меньше прошлого → «Счётчик не крутится назад — проверьте цифры».
- Ошибка на нашей стороне → «Что-то пошло не так на нашей стороне…» и «Повторить». В логе trace id.

Переписка целиком (генерирует `python -m tools.transcript`): [docs/DIALOG_EXAMPLE.md](docs/DIALOG_EXAMPLE.md).

## Известные ограничения

- Сроки, счета, права и передача в УК пока смоделированы (см. таблицу выше). Распознавание реальное, если заданы
  ключи YC: у водомеров примерно каждое четвёртое показание нужно поправить, поэтому бот всегда показывает
...
- ФГИС «Аршин»: API доступен только с российских IP (тесты — на реальных ответах ФГИС из `tests/fixtures/arshin/`
  и моках); если DNS в Docker не резолвит `fgis.gost.ru`, выручает `ARSHIN_FALLBACK_IPS`; лимит 2 запроса в секунду (бот держит паузу 0,6 с, до 4 запросов на проверку, кэш: найдено —
  30 дней, не найдено — 3 дня, после 3 сбоев подряд 10 минут не обращается); поверитель передаёт сведения до ~40
  рабочих дней, поэтому «не нашли» не значит «не поверен»; заводской номер уникален только внутри типа СИ —
  при коллизии бот спрашивает, какой прибор ваш, а не угадывает.
...
- Экраны: главная (дашборд, лист выбора адреса), подача (фото или вручную), счётчик с историей, профиль (адреса
  ...
  `add_address`, `delete_data`, а также `inv_new_<id>` (пригласить жильца) и `inv_acc_<id>` (список доступа).
